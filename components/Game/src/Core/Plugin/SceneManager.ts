...
class SceneManager {
    ...
    private _parentObjectList : CoreEntity[];
    ...
    public addObject(obj : IAbstractGameObject) {
        if (this._currentScene) {
            if (obj.parentObject != null) this._parentObjectList.push(<CoreEntity>obj.parentObject);
        ...
        if (this._pixiLayer.stage) {
            ...
            if (level) {
                for (let c = 0; c < level.children.length; c++) {
                    ...
                    (<any>window).dobject = obj;
                    obj.destroy();
                }

                for (let c = 0; c < this._parentObjectList.length; c++) {
                    console.log("Destroying!");
                    //Check this again
                    if ((<any>this._parentObjectList[c]).transform != null) {
                        this._parentObjectList[c].destroy();
                        console.log("destroyed parent list!")
                    }
                }
            }
