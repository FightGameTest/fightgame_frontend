...
class Loop {
    ...
    public addFunction(f : Function, context : any) {
        let fObj : IFunObj | null = this._getFunObj(f, context);
    ...
    private _getFunObj(f : Function, context : any) : IFunObj | null {
        for (let c = 0; c < this._fList.length; c++) {
    ...
    private _newFunObj(f : Function, context : any) : IFunObj {
