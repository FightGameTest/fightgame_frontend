...
class Scale {
    ...
    public resizeX(val : number) : number {
        if (this._scaleMode.sizeMode.x == Sizes.fill) {
            return this._preserveSign(this._getFillXScale(), val);
        } else if (this._scaleMode.sizeMode.x == Sizes.maintain_ratio) {
            return this._preserveSign(this._foreignObject.scale.y, val);
        } else {
            return this._preserveSign((val * this._x) * this._getRatio(), val);
        }
    }
    ...
    public resizeY(val : number) : number {
        if (this._scaleMode.sizeMode.y == Sizes.fill) {
            ...
            return this._preserveSign(this._getFillYScale(), val);
        } else if (this._scaleMode.sizeMode.y == Sizes.maintain_ratio) {
            return this._preserveSign(this._foreignObject.scale.x, val);
        } else {
            return this._preserveSign((val * this._y) * this._getRatio(), val);
        }
    }

    public convertPlaceX(val : number) : number {
        let base = this._getContainerXStart();
        let ratio = this._getRatio();
        let convertedVal = val * ratio;

        return convertedVal - base;
    }


    public convertPlaceY(val : number) : number {
    ...
    private _preserveSign(currentVal : number, originalVal : number) {
        if ((originalVal < 0 && currentVal > 0) || (originalVal > 0 && currentVal < 0)) {
            return currentVal * -1;
        } else {
            return currentVal;
        }
